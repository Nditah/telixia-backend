/**
 * @author 4Decoder
 * @description Talent holds record of all cities with terminals
 */
import Joi from "joi";
import log4js from "log4js";
import Talent, { schema, schemaFetch, schemaCreate, schemaUpdate } from "./model";
import { success, fail, notFound } from "../../lib/response";
import { STATUS_MSG } from "../../constants";

import { talentAuthenticate } from "../../services/authenticate";

// Logging
const logger = log4js.getLogger("[talent]");
log4js.configure({
    appenders: { file: { type: "file", filename: "logs/talent.log" } },
    ...
    try {
        if (id) {
            if (attributes.length > 0) {
                result = await Talent.findOne({ where: { id }, attributes });
            } else {
                result = await Talent.findOne({ where: { id } });
            }
        } else {
            result = await Talent.findAll(options);
        }
        if (!result) {
...
export async function createRecord(req, res) {
    ...
    try {
        const result = await Talent.create(data);
        if (!result) {
            logger.info(STATUS_MSG.SUCCESS.DEFAULT, []);
            return notFound(res, "Error: Bad Request: Model not found");
        }
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const model = await Talent.findByPk(id);
        if (!model) {
