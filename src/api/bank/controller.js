/**
 * @author 4Decoder
 * @description Bank holds record of all cities with terminals
 */
import Joi from "joi";
import log4js from "log4js";
import Bank, { schema, schemaFetch, schemaCreate, schemaUpdate } from "./model";
    ...
    try {
        if (id) {
            if (attributes.length > 0) {
                result = await Bank.findOne({ where: { id }, attributes });
            } else {
                result = await Bank.findOne({ where: { id } });
            }
        } else {
            result = await Bank.findAll(options);
        }
        if (!result) {
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const model = await Bank.findByPk(id);
        // const model = await Bank.findOne({ where: { id } });
        if (!model) {
