...
import Country, { schema, schemaFetch, schemaCreate, schemaUpdate } from "./model";
import { success, fail, notFound } from "../../lib/response";
import { STATUS_MSG } from "../../constants";

// Logging
const logger = log4js.getLogger("[country]");
log4js.configure({
    appenders: { file: { type: "file", filename: "logs/country.log" } },
    ...
    try {
        if (id) {
            if (attributes.length > 0) {
                result = await Country.findOne({ where: { id }, attributes });
            } else {
                result = await Country.findOne({ where: { id } });
            }
        } else {
            result = await Country.findAll(options);
        }
        if (!result) {
