...
import Setting, { schema, schemaFetch, schemaCreate, schemaUpdate } from "./model";
    ...
    try {
        if (id) {
            if (attributes.length > 0) {
                result = await Setting.findOne({ where: { id }, attributes });
            } else {
                result = await Setting.findOne({ where: { id } });
            }
        } else {
            result = await Setting.findAll(options);
        }
        if (!result) {
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const model = await Setting.findByPk(id);
        if (!model) {
