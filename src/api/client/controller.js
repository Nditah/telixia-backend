...
import { clientAuthenticate } from "../../services/authenticate";
    ...
    try {
        if (id) {
            if (attributes.length > 0) {
                result = await Client.findOne({ where: { id }, attributes });
            } else {
                result = await Client.findOne({ where: { id } });
            }
        } else {
            result = await Client.findAll(options);
        }
        if (!result) {
...
export async function updateRecord(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    try {
        const model = await Client.findByPk(id);
        if (!model) {
...
export async function login(req, res) {
    ...
    if (error) return fail(res, 422, `Error validating request data. ${error.message}`);
    return clientAuthenticate(req.body)
